...
| 4 | Security Gates | `make security-strict` / CI — падает на bad artifacts; отчёты в `artifacts/` |
| 5 | Жизненный цикл | `demo.sh` — 4 сценария без ручной магии |
...
| 9 | docker compose | `docker compose up -d` + `make demo` |
...
| CI/CD Security Gates | `make security-strict`, Actions §13 | Live: `evil_model.pkl` → exit 1 |
...
| UI | Streamlit §12 | Security Center vs CEO mock |
| docker compose + лёгкое демо | §8, `make demo` | С нуля на ноуте ≤15 мин |
...
| Auth / RBAC | Keycloak + OAuth2 Proxy (или mlflow-server-auth) | Роли, доступ к UI/API |
...
| Dashboard | Streamlit | Security Center + CEO mock |
...
make security-strict  # все обязательные — для демо и CI
```
...
| **G8** | Качество и **bias/robustness** (табличные) | Data leakage, performance drop, fairness, sanity tests на валидации | OWASP ML04, риск деградации в prod | Validation | Giskard | после train, до promote | scan failed / критичный test |
| **G9** | **Adversarial robustness** (табличные) | Устойчивость к adversarial examples (FGSM и т.п.) | Model evasion, extraction (частично) | Validation | ART | M1/M2 validation | robustness < порога |
| **G10** | Red-team **LLM/NLP** (offline) | Jailbreak, prompt leak, toxic output на тестовых промптах | T6 (prompt injection / unsafe LLM) | Validation | Garak | M3 перед promote | high-severity findings |
...
| **ceo** | страница CEO Report (мок) | mutate |

### 6.2. CEO Report (шутка из ТЗ)

Отдельная страница Streamlit: зелёные галочки, текст «всё супер», без привязки к реальным алертам.  
Реальный статус — только на вкладке **Security Center** (для MLSecOps).

### 6.3. Демо-пользователи (создаёт `bootstrap.sh`)
...
| `ceo` | ceo | из `.env` | страница CEO Report |

Keycloak realm: `mlsecops`. Клиенты: `mlflow`, `dashboard` (confidential или public+PKCE для Streamlit).

### 6.4. Матрица действий (кто что может)

| Действие | ds | mlsecops | de | product | ceo |
|----------|:--:|:--------:|:--:|:-------:|:---:|
...
| View audit / findings | ✓ | ✓ | ✓ | ✓ | ✓ (CEO — только mock) |
...
Реализация: `platform/audit.py` — `log_event()`, `log_finding()`, `verify_chain()`.
...
### 7.6. Пример записей после `make demo`
...
| Сервис | Порт (пример) | Зависимости |
|--------|---------------|-------------|
| `postgres` | 5432 | — |
| `minio` | 9000, 9001 | — |
| `keycloak` | 8080 | postgres |
| `mlflow` | 5000 | postgres, minio |
| `oauth2-proxy` | 4180 | keycloak, mlflow |
| `api-scoring` | 8001 | mlflow, postgres |
| `api-antifraud` | 8002 | mlflow, postgres |
| `litellm` | 4000 | — |
| `llm-guard-sidecar` или встроен в api | — | litellm |
| `dashboard` | 8501 | postgres, mlflow |
...
make bootstrap                       # keycloak users, buckets, mlflow exp
make train-all                       # или загрузка pretrained
make security-strict
make demo                            # полный сценарий
```
...
| dashboard | `curl :8501/_stcore/health` | 200 |

Если Keycloak долго стартует — `make bootstrap` с retry (sleep 30).

### 8.5. Порты и URL для демо (localhost)

| URL | Назначение |
|-----|------------|
| http://localhost:8501 | Streamlit Security Center |
...
| 3 | Обучение | `make train-m*`, MLflow experiment | G0,G1,G3 на коде; lineage tags |
...
- Запуск: `make demo` из корня репозитория (после `docker compose up` + `bootstrap`).
...
| 5 | `make security-strict --model m1` | exit 0, tags `security.G5=passed` |
...
1. `make train-m1` (лог в MLflow; lineage: `dataset_version`, `git_commit`).
2. Gates G8, G9.
...
| 14–15 | Вопросы | CEO page как ирония; SecAI = фаза 2 |
...
| Gates «Run strict» | subprocess `make security-strict` (осторожно: долго) |

---

## 13. CI/CD

### 13.1. GitHub Actions (`.github/workflows/security.yml`)
...
1. `secrets` — gitleaks  
2. `sast` — semgrep + bandit (опц.)  
3. `dependencies` — pip-audit + guarddog  
4. `model-scan` — modelaudit на `models/**` и `artifacts/**`  
5. `upload-sarif` — артефакты в Actions (опц.)

Deploy job **не** переводит модель в Production — только `mlsecops` через `promote_to_production.py` после локальных/ручных gates.

### 13.2. Makefile (целевые цели)

```makefile
.PHONY: up down bootstrap train-all security-fast security-strict demo promote

up: docker compose up -d --build
down: docker compose down
bootstrap: scripts/bootstrap.sh
train-all: ...
security-fast: scripts/run_gates.sh --profile fast
security-strict: scripts/run_gates.sh --profile strict
...
| 5 | `make demo` | exit 0, ≥20 audit rows |
| 6 | `wc -l docs/threat_model.md` | ≥150 строк содержательного текста |
| 7 | `make security-strict` | all mandatory green |
...
**Триггер:** работают compose, 3 модели, gates, audit, `demo.sh`, Streamlit.
...
- [ ] `make demo` проходит без ручных правок  
...
- [ ] CEO page — мок, Security Center — реальные данные  
...
- ingest: `scripts/ingest_dataset.py` → register в `datasets` → только `status=available` разрешает `make train-*`.
...
| Раздел | Изменение |
|--------|-----------|
| §7 Audit | hash-chain в `platform/audit.py`; helper `verify_audit_chain()` |
...
**В: CEO Report — баг?**  
О: Нет, **намеренный мок** из ТЗ; реальность — Security Center.
...
- [ ] `platform/audit.py` (hash-chain + findings)
